...
[ "$UNREAD_CODE" = "200" ] \
  && pass "Header 鉴权：/api/message/unread-count code=200" \
  || fail "Header 鉴权：/api/message/unread-count 返回 code=${UNREAD_CODE}，预期 200"

# 2b. 评论列表（登录态，任意 postId，只验证鉴权不返回 401）
COMMENT_HTTP=$(curl -sf -o /dev/null -w "%{http_code}" \
  "$BASE_URL/api/comment/list?postId=000000000000000000000000&page=1&size=5" \
  -H "Authorization: Bearer $TOKEN")
[ "$COMMENT_HTTP" != "401" ] \
...
UNREAD_COOKIE_CODE=$(echo "$UNREAD_COOKIE_RESP" | python3 -c \
  "import sys,json; print(json.load(sys.stdin).get('code', -1))" 2>/dev/null || echo "-1")
[ "$UNREAD_COOKIE_CODE" = "200" ] \
  && pass "Cookie 鉴权：/api/message/unread-count code=200" \
  || fail "Cookie 鉴权：/api/message/unread-count 返回 code=${UNREAD_COOKIE_CODE}，预期 200"
...
SEARCH_HTTP=$(curl -sf -o /dev/null -w "%{http_code}" \
  "$BASE_URL/api/post/search?keyword=test&page=1&size=5")
[ "$SEARCH_HTTP" != "401" ] \
  && pass "无 Token：公开搜索接口不返回 401（HTTP ${SEARCH_HTTP}）" \
  || fail "无 Token：公开搜索接口返回 401，不应要求鉴权"

# ---- 5. 无 Token：需鉴权接口应返回 401 ----
NOTIF_HTTP=$(curl --noproxy '*' -sS -o /dev/null -w "%{http_code}" \
